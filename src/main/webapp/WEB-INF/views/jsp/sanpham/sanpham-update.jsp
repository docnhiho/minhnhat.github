					...
					<div class="container-fluid">
						...
						<div class="card shadow mb-5">
							<div class="card-header py-3">
								<h6 class="m-0 font-weight-bold text-primary">Update Sản
									Phẩm</h6>
							</div>
							<c:url value="/updateSanpham" var="updateSanpham" />
							<form:form id="sanphamUpdate" action="${updateSanpham}"	 modelAttribute="sanpham"
								...
								<div class="form-row" style="padding: 20px">
									<div class="form-group col-md-6">
										...
										<label>Loại Sản Phẩm</label> <select
											class="form-control" name="loaisanpham">
											<c:if test="${not empty listLoaisp}">
												<c:forEach var="lsp" items="${listLoaisp}">
													<option value=${lsp.id } selected>${lsp.tenloaisp}</option>
												</c:forEach>
											</c:if>
										</select> <br /> <br /> 
										...
										<form:input class="form-control" path="tensanpham"
											...
											pattern="[a-zA-Z]\{25}" title="Tên sản phẩm" />

										<br /> <br /> 
										
										<label>Nha Cung Cấp</label> <select
											class="form-control" name="nhacungcap">
											<c:if test="${not empty listNhacungcap}">
												<c:forEach var="ncc" items="${listNhacungcap}">
													<option value=${ncc.id } selected>${ncc.tennhacungcap}</option>
												</c:forEach>
											</c:if>
										</select>
									</div>
									...
									<div class="form-group col-md-6">
										...
										<form:input class="form-control"   path="gia"
											...
											pattern="[0-9]{5,7}" title="Giá" />										
										<br /> <br /> <label>Mô tả</label>
										<form:input class="form-control" path="mota"
											placeholder="Nhập mô tả" required="required"
											pattern="[a-z0-9]\{50}" title="Mô tả" />
